...
CREATE TABLE `user` (
  ...
  `must_change_pwd` tinyint(1) DEFAULT '0' COMMENT '是否必须修改密码：1=首次登录须强制设置新密码，0=正常',
  `created` datetime DEFAULT NULL,
  `updated` datetime DEFAULT NULL,
  `version` int(11) DEFAULT '0',
  PRIMARY KEY (`id`)
...
-- ----------------------------
-- Table structure for fastcms_install
-- ----------------------------
DROP TABLE IF EXISTS `fastcms_install`;
...
INSERT INTO `user` (`id`, `user_name`, `nick_name`, `password`, `salt`, `must_change_pwd`, `created`, `updated`) VALUES ('1', 'admin', 'admin', '$2a$10$Lpudyy6BI./H9UJc9eIPjuflK4g.A.CnwCb1qgE2PGbWyjv2yDfbq', '1622734716287', 1, NOW(), NOW());
INSERT INTO `user_role` (`user_id`, `role_id`) VALUES ('1', '1');
INSERT INTO `fastcms_install` (`id`, `install_time`, `version`, `created`, `updated`) VALUES ('1', NOW(), '0.2.0', NOW(), NOW());
...
CREATE TABLE `ai_template_message` (
  ...
  `prompt_tokens` int DEFAULT NULL COMMENT '本轮输入token（仅assistant，跨轮次聚合）',
  `completion_tokens` int DEFAULT NULL COMMENT '本轮输出token（仅assistant）',
  `total_tokens` int DEFAULT NULL COMMENT '本轮总token（仅assistant）',
  `created` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idx_session_id` (`session_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='AI 模板生成对话消息表';

-- 已有环境升级：为 ai_template_message 增加 token 用量字段（每轮 assistant 消息回写，hover 展示）
